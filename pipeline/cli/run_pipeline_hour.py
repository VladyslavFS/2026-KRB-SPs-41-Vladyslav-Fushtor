...
from pipeline.cli.load_bi_store import main as load_bi_main
...
def main() -> None:
    ...
    ap.add_argument("--skip-load", action="store_true", help="Skip loading to BI Store")
    ...
    if args.skip_gold or args.skip_export:
         print("=== [3/4] build_gold_layer (SKIPPED) ===")
    else:
        print("=== [3/4] build_gold_layer ===")
        sys.argv = ["build_gold_layer", "--days", str(int(args.export_days))]
        build_gold_main()

    # 4) Load BI Store (S3 -> Postgres)
    if args.skip_load:
         print("=== [4/4] load_bi_store (SKIPPED) ===")
    else:
        print("=== [4/4] load_bi_store ===")
        sys.argv = ["load_bi_store", "--days", str(int(args.export_days))]
        load_bi_main()
